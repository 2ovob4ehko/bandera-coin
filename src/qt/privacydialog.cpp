...
#include "zbanderacontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBANDERA ought to be enough for anybody." - Bill Gates, 2017
    ui->zBANDERApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBANDERASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBANDERA->setEnabled(false);
        ui->pushButtonMintzBANDERA->setToolTip(tr("zBANDERA is currently disabled due to maintenance."));

        ui->pushButtonSpendzBANDERA->setEnabled(false);
        ui->pushButtonSpendzBANDERA->setToolTip(tr("zBANDERA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBANDERApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBANDERA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBANDERA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBANDERA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBANDERA in ") + 
...
void PrivacyDialog::on_pushButtonSpendzBANDERA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBANDERA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBANDERA
        sendzBANDERA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBANDERA
    sendzBANDERA();
}

void PrivacyDialog::on_pushButtonZBanderaControl_clicked()
{
    ZBanderaControlDialog* zBANDERAControl = new ZBanderaControlDialog(this);
    zBANDERAControl->setModel(walletModel);
    zBANDERAControl->exec();
}

void PrivacyDialog::setZBanderaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBANDERASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBANDERA()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bandera Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBANDERApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBANDERA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BANDERA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBANDERA</b>";
    ...
    // use mints from zBANDERA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBanderaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBanderaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbandera selector in case it was used
    ZBanderaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBANDERA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bandera, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBANDERA Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zBANDERApayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBANDERA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBANDERA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBANDERA "));
    ui->labelzBANDERAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBANDERASyncStatus->setVisible(fShow);
}
